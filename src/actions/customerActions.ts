...
export async function createCustomer(data: customerTypes) {
  try {
    ...
    if (!data.customer_id) {
      data.customer_id = await generateCustomId(
        Customers,
        "customer_id",
        "customerId",
      );
...
export async function updateCustomer(
  ...
  try {
    ...
    const byObjectId = mongoose.Types.ObjectId.isValid(customerIdOrObjectId)
    ...
    if (
      ...
      updates.mobile_number !== customer.mobile_number
    ) {
    ...
    const filteredUpdates = Object.fromEntries(
      Object.entries(updates).filter(
        ([k, v]) =>
          (allowed as readonly string[]).includes(k) && v !== undefined,
      ),
